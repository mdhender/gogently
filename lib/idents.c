...
PRIVATE void allocate_idtab () {
    idtab_ptr = (struct IDENTSTRUCT *) malloc (sizeof (IDTAB /*struct IDENTSTRUCT [IDTABPIECESIZE]*/ ) );
    if (idtab_ptr == 0) {
        printf("memory allocation failed\n");
        exit(1);
    }
...
void slice_to_id (char *idstart /* position of first character */, char *idstop /* position  a f t e r  last character */, IDENT *ref_id) {
    long  hash, length;
    IDENT chain;
    IDENT  NewId;
...
/*--------------------------------------------------------------------*/
ErrorI (char *str1, IDENT id, char *str2, long pos) {
    char *idrepr;
    char buf[300];
