...
void GetSourceName (char ** str) {
    *str = InFileName;
}

/*----------------------------------------------------------------------------*/

static FILE * open_file (char * unit) {
    char buf[1000];
    FILE * InFile;
...
static long is_defined(char * path) {
    long i;
...
#define RECOG(x) {                                    \
        bufptr = BUFPTR;                              \
        yylval.attr[0] = CurFile * MaxLines * MaxCols + CurLine * MaxCols + (BUFPTR - firstcolptr); \
        return(x);                                    \
    }
...
void PosToLineNumber (long Pos, long *N) {
    *N = GetLine(Pos);
}

/*----------------------------------------------------------------------------*/

long GetCol (long Pos) {
