...
static allocate_idtab () {
    idtab_ptr = (struct IdentRec *) malloc (sizeof (IDTAB /*struct IdentRec [IDTABSIZE]*/ ) );
    if (idtab_ptr == 0) {
...
slice_to_id (char *idstart /* position of first character */, char *idstop;  /* position  a f t e r  last character */, Ident *ref_id) {
    long  hash, length;
    Ident chain;
    Ident  NewId;
    length = idstop-idstart;
    hash = ( length*256 + ((*idstart)&0xf)*16 + (*(idstop-1)&0xf) ) & (HashTabSize-1);
    chain = HashTab[hash];
...
string_to_id (char *idstart, Ident *ref_id) {
    char *idstop;
    idstop = idstart;
    while (*idstop != '\0') {
        idstop++;
...
id_to_string (Ident id, char **repr) {
    *repr = id->firstposptr;
}
...
DefGlobalMeaning (Ident id, long m, long f) {
    id->GlobalMeaning = m;
    id->ExportFlag = f;
}
...
DefFunctorMeaning (Ident id, long m) {
    id->FunctorMeaning = m;
}

/*----------------------------------------------------------------------------*/

DefLocalMeaning (Ident id, long m1, long m2) {
    id->LocalMeaning = m1;
    id->LocalMeaning2 = m2;
}
...
ForgetLocalMeaning (Ident id) {
    id->LocalMeaning = 0;
}

/*----------------------------------------------------------------------------*/

long GetGlobalMeaning (Ident id, long *m) {
...
long GetExportFlag (Ident id, long *f) {
    if (id->GlobalMeaning == 0){
        return 0;
    }
    *f = id->ExportFlag;
...
long GetFunctorMeaning (Ident id; long *m) {
...
long GetLocalMeaning (Ident id, long *m) {
...
long GetLocalMeaning2 (Ident id, long *m) {
    if (id->LocalMeaning == 0){
        return 0;
    }
    *m = id->LocalMeaning2;
...
long HasGlobalMeaning (Ident id) {
    return id->GlobalMeaning != 0;
}

/*----------------------------------------------------------------------------*/

long HasLocalMeaning (Ident id) {
    return id->LocalMeaning != 0;
}

/*----------------------------------------------------------------------------*/

long get_symbol_class (Ident Id) {
    return Id->symbol_class;
}

/*----------------------------------------------------------------------------*/

void enter_keyword (int class, char *idstart) {
    Ident NewId;
...
void UniqueTokenId (Ident *ref_id) {
    char buffer[100];
    tokencount++;
...
void CreateNameForToken (char *Str, Ident *ref_id) {
    clear_buffer();
    if (is_word(Str)) {
        append_to_buf("yykw_");
        append_to_buf(Str);
